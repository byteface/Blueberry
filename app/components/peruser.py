...
class Peruser(object):
    ...
    def create_contents(self) -> str:
        ...
        if sys.platform == "win32":
            ...
            for c, line in enumerate(ls(f"-alp {self.dir}")):
                ...
                name = line.split(" ")[-1]
                if name == "./" or name == "../":
                    continue  # dont draw mac hidden dirs
                if "/" in name:
    ...
    def create_folder(self, name: str):
        ...
        el = div(
            ...
            **{"_data-id": self.id},
        ).html(div(span("📁"), _style="font-size:70px;", _class="icon"), h5(name))
        return el
                ...
                div(_id="view").html(
                    div(_id="sidebar").html(
                        ul(
                            ...
                            li(
                                span("👆 Uploads", _onclick=Peruser.UPLOADS_DIR(self.id))
                            ),  # TODO - get from config
                            li(
                                span("Files", _class="group"),
