...
class Launcher(object):
    ...
    def __str__(self):
        ...
        b1 = button(
            "TEST LAUNCH PLAYER", _onclick="add_to_page('/component/player?id=player')"
        )
        ...
        b2 = button(
            "TEST LAUNCH UPLOADER",
            _onclick="add_to_page('/component/upload?id=upload')",
        )
        ...
        return str(
            div(_id=self.id).html(
                ...
                button(
                    "TEST LAUNCH UPLOADER", _onclick=escape(f"add_to_page({link2})")
                ),
                script(
                    """
                $("#"""
                ...
                $(".launcher_overlay").on( "click", function(){ $("#"""
                    + self.id
                    + """").remove() } );
